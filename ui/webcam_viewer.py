...
class WebcamViewer(QWidget):
    ...
    flip_toggled = Signal(
        bool
    )  # NEW: lets MainWindow sync flip state to the processing pipeline
    ...
    def setup_ui(self):
        ...
        # ==========================================
        # Bottom Controls — just Flip + FPS now
        # ==========================================
        ...
        self.flip_btn.setFocusPolicy(Qt.FocusPolicy.NoFocus)
    ...
    def _on_flip_clicked(self, checked: bool):
        self.flipped = checked
        self.flip_toggled.emit(
            checked
        )  # notify MainWindow so it can sync to FrameReader
    ...
    def pause_camera(self):
        """Stop the local preview loop WITHOUT clearing current_camera.
        Used when handing control to the Processor pipeline."""
        self.timer.stop()
        if self.cap:
            self.cap.release()
            self.cap = None
    ...
    def resume_camera(self):
        """Reopen the camera for local preview after processing stops."""
        if self.current_camera and self.current_camera != "No cameras found":
