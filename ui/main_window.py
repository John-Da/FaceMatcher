...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.webcam_viewer.flip_toggled.connect(self._on_flip_toggled)
        ...
        self.processor.fps_ready.connect(
            lambda fps: self.webcam_viewer.fps_label.setText(f"FPS: {fps:.1f}")
        )

        self.left_panel.export_btn.clicked.connect(self.export_results)

        self.left_panel.threshold_slider.valueChanged.connect(
            self._on_threshold_changed
        )

    def _on_threshold_changed(self, value: int):
        if self.processor.isRunning():
            self.processor.threshold = value / 100.0

    def _on_flip_toggled(self, checked: bool):
        if self.reader:
            self.reader.flipped = checked
    ...
    def _begin_processing(self):
        ...
        if self.left_panel.image_radio.isChecked():
            ...
            self.webcam_viewer.pause_camera()   # <-- ADD THIS: stop local preview loop
        ...
        if source == "image":
            ...
            self.reader.flipped = self.webcam_viewer.flipped
    ...
    def stop_processing(self):
        ...
        if self.left_panel.webcam_radio.isChecked():
            self.webcam_viewer.resume_camera()   # <-- ADD THIS: give preview control back
